...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, axed):
        golist = axed.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_axed(axed, item)
    ...
    @classmethod
    def import_gobject_from_axed(self, axed, rec):
        import decimal
        import axelib
        import inflection
        ...
        # shim/axed conversion
        object_hex = axelib.SHIM_deserialise_from_axed(object_hex)
        objects = axelib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from axed...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from axed, with every run
        ...
        # get/create, then sync payment amounts, etc. from axed - Axed is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(axed)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from axed! %s" % e)
            govobj.vote_delete(axed)
            return (govobj, None)
    ...
    def vote_delete(self, axed):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(axed, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, axed, signal, outcome):
        import axelib
        ...
        output = axed.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = axelib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(axed, signal)

    def sync_network_vote(self, axed, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = axed.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = AXED_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 axe addr, non-multisig
            if not axelib.is_valid_axe_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Axe address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (axelib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/AxeDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import axelib
        obj_data = axelib.SHIM_serialise_for_axed(self.serialise())
    ...
    def prepare(self, axed):
        try:
            object_hash = axed.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = AXED_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not axelib.is_valid_axe_address(addr, config.network):
    ...
    def hash(self):
        import axelib
        return axelib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = AXED_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, axed):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - axed.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, axed):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - axed.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, axed):
        now = int(time.time())
        return (self.created_at < (now - axed.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, axed):
        if self.is_expired(axed):
            return False

        return True

    def is_deletable(self, axed):
        if self.is_expired(axed):
            return True
