...
class AxeDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some axed methods
    ...
    @classmethod
    def from_axe_conf(self, axe_dot_conf):
        from axe_config import AxeConfig
        config_text = AxeConfig.slurp_config_file(axe_dot_conf)
        creds = AxeConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = axelib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = axelib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = axelib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_axed_proto_version_with_sentinel_ping)
