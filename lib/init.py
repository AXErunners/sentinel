...
def has_axe_conf():
    ...
    valid_axe_conf = False

    # ensure axe_conf exists & readable
    #
    # if not, print a message stating that Axe Core must be installed and
    # configured, including JSONRPC access in axe.conf
    try:
        f = io.open(config.axe_conf)
        valid_axe_conf = True
    except IOError as e:
        print(e)

    return valid_axe_conf
...
def main():
    ...
    if not has_axe_conf():
        print("AxeCore must be installed and configured, including JSONRPC access in axe.conf")
        sys.exit(1)
