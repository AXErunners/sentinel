...
class AxeConfig():
    ...
    def slurp_config_file(self, filename):
        # read axe.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
        ...
        # data is axe.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        ...
        default_port = 9337 if (network == 'mainnet') else 19337

        # use default port for network if not specified in axe.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from axe.conf
