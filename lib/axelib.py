...
def is_valid_axe_address(address, network='mainnet'):
...
# shims 'til we can fix the axed side
def SHIM_serialise_for_axed(sentinel_hex):
    from models import AXED_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = AXED_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in axed
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # axed expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    axed_hex = serialise(obj)
    return axed_hex


# shims 'til we can fix the axed side
def SHIM_deserialise_from_axed(axed_hex):
    from models import AXED_GOVOBJ_TYPES

    # unpack
    obj = deserialise(axed_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('axe.conf')
